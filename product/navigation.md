# Poladin — Navigation Specification
...
`▦لوگو` → `محصولات ▾` → `قیمت‌ها` → `پولادین` → `ابزارها ▾` → `وبلاگ/اخبار` → `باشگاه` → `همکاری` → `تماس` … then utility (§6): `طلا و ارز` · `🔍` · `ورود/حساب`.

### Behavior
- **Sticky:** on scroll-down past the ticker, the header **condenses** into a sticky bar: `▦لوگو · 🔍/پولادین · حساب · ☰(secondary)`. On scroll-up, full header returns. Ticker scrolls away with the page (re-appears at top).
- **Active top-level:** the section matching the current route is marked (Cobalt underline + `aria-current="page"`).
- **«پولادین»** entry opens the AI (hero focus on home, full AI view elsewhere).
...
- **Layout:** full-width panel, **7 category columns**; each column = category title (link) + its sub-categories (links) + a small «قیمت روز» CTA. A trailing **«بپرس از پولادین»** column with the AI chip + a featured price.
...
3. **شرکت:** چرا پولادین · درباره ما · تماس · همکاری با ما.
...
- **SKU:** related SKUs (same sub-category, other sizes/factories) + «مقالهٔ مرتبط» + «پرسش از پولادین».
...
`خانه` · `قیمت‌ها` · **`🟠 پولادین`** (center, elevated, amber spark) · `سبد استعلام` · `حساب`
...
- Mirrors header + footer: محصولات (accordion → categories → sub-categories) · قیمت‌ها · پولادین · ابزارها · وبلاگ/اخبار · باشگاه · همکاری · درباره/تماس · (طلا و ارز) · ورود/حساب · channels.
- `role="dialog"`, **focus-trapped**, `Esc`/overlay closes, focus returns to the toggle; swipe-to-close.

---

## 14. N14 · AI as Navigation
- The AI «پولادین» is a **first-class navigation method**: a natural-language query routes the user to a price/table/tool/quote. It can deep-link («صفحهٔ میلگرد ۱۴ ذوب‌آهن را باز کن») and always offers structured next steps (chips). Present as the hero (home) and a FAB (everywhere on mobile).
