# Poladin — Acceptance Criteria (Master Specification)
...
**AC-B-3 — Filters/search** — narrowing by size/grade/factory updates rows in < 300ms client-side; empty result shows a Persian empty state with a "ثبت درخواست/پرسش از پولادین" CTA.
...
**AC-B-6 — Exports**
- **AC-B-6a Excel:** produces a valid `.xlsx` of the current (filtered) table with headers, Jalali date, and a Poladin header row.
...
## 5. AI Advisor «پولادین» (Epic D) — rigorous (critical)
...
**AC-J-1** «چرا پولادین؟», mill logos «تأمین‌کنندگان», customer logos «مشتریان» render; missing logo → branded placeholder.
