# Fooladno — Acceptance Criteria (Master Specification)
...
**AC-B-3 — Filters/search** — narrowing by size/grade/factory updates rows in < 300ms client-side; empty result shows a Persian empty state with a "ثبت درخواست/پرسش از فولادنو" CTA.
...
**AC-B-6 — Exports**
- **AC-B-6a Excel:** produces a valid `.xlsx` of the current (filtered) table with headers, Jalali date, and a Fooladno header row.
...
## 5. AI Advisor «فولادنو» (Epic D) — rigorous (critical)
...
**AC-J-1** «چرا فولادنو؟», mill logos «تأمین‌کنندگان», customer logos «مشتریان» render; missing logo → branded placeholder.
