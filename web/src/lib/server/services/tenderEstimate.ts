...
export async function factoryOptionsFor(
  ...
  const opts = matched.map((r) => {
    ...
    return {
      ...
      size: r.size,
      unit: r.unit,
      // Explicitly null for every non-kilogram basis, not 1: «۱ عدد کوپلر» is
      // not one kilogram and «۱ کلاف لوله مسی» is not either — those prices
      // are per whole item, so there is no kg bridge to advertise. Consumers
      // already render a null as «وزن نامشخص».
      weightKgPerUnit:
        r.priceBasis !== 'kg' ? null : r.unit === 'kg' ? 1 : (r.theoreticalWeightKg ?? null),
