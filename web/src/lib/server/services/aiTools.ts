      ...
      case 'getPrice': {
        ...
        return {
          ...
          results: rows.map((r) => ({
            ...
            unit: r.current.unit,
            // Distinct from `unit`: a SKU can be quoted per-piece/branch while
            // its stored price is actually denominated per kilogram (J-218) —
            // without this the model has no way to know the two can diverge
            // and would state `unit` alone as if it were the price's basis.
            priceBasis: r.current.priceBasis,
...
3-ج) واحد پول همیشه و فقط تومان است؛ هرگز کلمهٔ «ریال» را برای هیچ قیمتی به کار نبر، حتی اگر کاربر خودش «ریال» بگوید — در آن حالت بگو منظورت را بر حسب تومان تکرار می‌کنی و همان عدد تومان را بگو. خروجی getPrice دو فیلد جدا دارد: unit (واحد شمارشی مثل شاخه/کیلوگرم) و priceBasis (مبنای واقعی قیمت). وقتی این دو یکی نیستند (مثلاً unit=«branch» ولی priceBasis=«kg»)، هرگز فقط یکی را نگو؛ صریح بگو «قیمت هر شاخه بر مبنای وزن (کیلوگرم) محاسبه می‌شود» تا مشتری این دو را قاطی نکند.
