...
/**
 * G-164 (single-delete path): the same atomic open-order guard the
 * bulk-delete route got (`deleteSkuGuarded`, replacing the old two-query
 * `skuImpact()` + `deleteSku()`) — this proves the route wires it correctly:
 * blocked without override, deletes with it.
 */
...
function deleteReq(id: string, override = false) {
  const url = `http://localhost/api/admin/catalog/skus/${id}${override ? '?override=true' : ''}`;
  return new NextRequest(url, { method: 'DELETE' });
}

describe('DELETE /api/admin/catalog/skus/[id] — open-order guard, real DB (G-164)', () => {
  it('blocks a delete on an open order unless override=true, then actually deletes on override', async () => {
    ...
    try {
      ...
      const blocked = await DELETE(deleteReq('sku-open'), { params: Promise.resolve({ id: 'sku-open' }) });
      expect(blocked.status).toBe(409);
      const blockedBody = await blocked.json();
      expect(blockedBody.error).toBe('open_orders');
      expect((await db.select().from(schema.skus).where(eq(schema.skus.id, 'sku-open'))).length).toBe(1);

      const ok = await DELETE(deleteReq('sku-open', true), { params: Promise.resolve({ id: 'sku-open' }) });
