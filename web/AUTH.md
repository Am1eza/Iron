...
| 59  | **User Profile**    | `/api/me`, `/api/me/profile` (PUT); `<ProfileForm>` + `<LogoutButton>`; `/account` dashboard                                                                               |
...
Browser (periodically) ─POST /api/auth/refresh▶ rotate refresh (single-use) → new access cookie
...
Guards: **server** pages use `requireUser()` / `requirePermission()` (guests → OTP login; unauthorized staff routes → 404, hidden not revealed), and `middleware.ts` gates `/admin/*` in the Node middleware runtime from `ADMIN_PATH_PERMISSIONS`. The server always enforces; **every client-side check below is UX only** — it decides which controls are worth rendering, never whether an action is allowed.
...
Rules that both a route handler and the UI need to agree on live in `lib/auth/roles.ts` as pure, id-based functions (`can`, `canAccessAdmin`, `canChangeLeadAssignee`) — the UI asks the _same_ function as the API, so a button can never appear that the server would answer 403 to.
...
Modified: `stores/auth.ts` (+role, loading status), `api/resources/auth.ts` (real endpoints), `LoginForm.tsx` (name + dev hint), `layout.tsx` (server session → AuthHydrator), `middleware.ts` (cookie name), `validation/{api,schemas}.ts`, `package.json` (jose), `.env.example`.

> **Historical deployment observation.** `SESSION_SECRET`, `SMSIR_API_KEY`/`SMSIR_TEMPLATE_ID` and `AUTH_ENFORCED=true` are set in production, and the DB-backed store (`store.pg.ts`) is what actually runs. Do **not** re-implement it.
...
_Ahantime — اول مشورت، بعد خرید._
