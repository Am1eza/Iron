...
**Builds on:** `STATE-MANAGEMENT.md`, `FORMS.md`, `product/data-model.md §10`.
...
3. **Resilient.** Timeouts (AbortController), retry-with-backoff for idempotent GETs, friendly Persian errors. Caller cancellation preserves its abort reason and stops retries; malformed responses fail without automatic retry.
4. **Validated at the boundary.** External/backend responses are parsed (Zod); bad data → graceful fallback, not a crash.
5. **SSR-aware.** Absolute base URL on the server, relative in the browser; browser cookies included (`credentials: 'include'`). Server-side credentials require explicit forwarding.
...
- **Forms:** import the typed resource directly, e.g. `authApi` or `contactApi`. Forwarding-only form and compatibility façades have been removed.
- **Server (RSC):** resources are isomorphic; on the server `BASE_URL` is absolute. `credentials: include` does not forward an incoming server request’s cookies automatically; session-aware server calls need explicit propagation.
...
- `credentials:'include'` for the session cookie; never put secrets in client code (AI relay/SMS keys live in route handlers). Payloads validated server-side (`VALIDATION`). Timeouts bound hung requests.

## 10. Conventions

- Components/hooks prefer direct typed resource imports. The `api.*` aggregate remains for existing consumers. New endpoints add a typed resource action + (if external) a response schema.
