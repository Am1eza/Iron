...
`NEXT_PUBLIC_SITE_URL`, `NEXT_PUBLIC_API_MODE`, `DEEPSEEK_API_KEY` + `DEEPSEEK_BASE_URL` (relay), `SMSIR_API_KEY` + `SMSIR_TEMPLATE_ID`, `BRSAPI_KEY`, `JWT/SESSION_SECRET`. Secrets are server-only (no `NEXT_PUBLIC_` prefix).
