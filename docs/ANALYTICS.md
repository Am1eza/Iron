...
## ۳) Google Analytics 4 / Tag Manager — ریسک بررسی و پذیرفته شد، فعال‌سازی در جریان ✅

کد پشتیبانی‌اش در `components/analytics/Analytics.tsx` هست و با ست‌کردن
`GTM_ID` (نه `GA4_ID` مستقیم — پایین را بخوانید) + **rebuild** فعال می‌شود
(این دو دامنه‌های CSP را باز می‌کنند، پس زمان build لازم دارند).

این تصمیم قبلاً آگاهانه غیرفعال نگه داشته شده بود، به همین دو دلیل واقعی:
...
**۲۶ مرداد ۱۴۰۵ — مالک این ریسک را دید و آگاهانه پذیرفت**: اکانت گوگل از طریق
یک VPN با IP ثابت مدیریت می‌شود (کاهش ریسک تعلیق حساب)؛ ریسک عدم‌دسترسی بخشی
از بازدیدکننده‌های ایرانی به `googletagmanager.com` همچنان باقی و پذیرفته‌شده
است. طبق توصیهٔ یک کارشناس مارکتینگ، **GTM** فعال می‌شود (نه `GA4_ID` مستقیم)
— تگ GA4 Configuration داخل خود کانتینر GTM تعریف می‌شود، همان‌طور که پایین‌تر
توصیه شده بود. Matomo دست‌نخورده و همچنان سیستم اصلی و پایدارتر باقی می‌ماند؛
GA4/GTM یک لایهٔ موازی برای مارکتینگ/تبلیغات است، نه جایگزین.

رویدادهای تبدیل (`lead`/`ai-chat`/`contact`) از همان ۷ نقطه‌ی فراخوانی موجود
(`web/src/lib/analytics/track.ts`) به‌صورت خودکار هم به Matomo هم به
`dataLayer` (برای GTM) فرستاده می‌شوند — کد تغییری در فرم‌ها لازم نداشت.
...
GA4_ID=                # خالی نگه دارید — از GTM_ID استفاده کنید، بخش ۳ را بخوانید
GTM_ID=                # مقدار Container ID واقعی (GTM-XXXXXXX) را اینجا بگذارید
```
